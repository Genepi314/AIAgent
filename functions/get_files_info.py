...
def get_files_info(working_directory, directory="."):
    ...
    try:
        ...
        # Get info about target directory
        target_dir_list = os.listdir(target_dir)
        working_dir_content = {}
        result_string = ""
        for filename in target_dir_list:
            temp_path = os.path.join(target_dir, filename)
            working_dir_content[filename] = {
                    "file_size": os.path.getsize(temp_path),
                    "is_dir": os.path.isdir(temp_path)
            }
            inner_dict = working_dir_content[filename]
            result_string += f"- {filename}: file_size={inner_dict["file_size"]}, is_dir={inner_dict["is_dir"]}\n"
